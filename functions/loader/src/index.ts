...
cloudEvent('loader', async (event: CloudEvent<MessagePublishedData>) => {
  ...
  try {
    ...
    if (!curatedExists) {
      // First load: copy staging table to curated with partitioning + clustering
      const [copyJob] = await bigquery.createJob({
        configuration: {
          copy: {
            sourceTable: {
      ...
      await copyJob.getQueryResults();

      // Set partitioning and clustering on the new table
      const [metadata] = await curatedTable.getMetadata();
      metadata.timePartitioning = { type: 'DAY' };
      if (clusteringFields.length > 0) {
        metadata.clustering = { fields: clusteringFields };
      }
      await curatedTable.setMetadata(metadata);
    } else {
